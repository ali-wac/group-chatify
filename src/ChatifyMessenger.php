...
use Chatify\Http\Models\GroupMessageSeen;
...
class ChatifyMessenger
{
    ...
    public function messageCardGroup($data, $viewType = null){
        $data['viewType'] = ($viewType) ? $viewType : $data['viewType'];
        return view('Chatify::layouts.messageCardGroup',$data)->render();
    }
    ...
    public function fetchMessagesQuery($user_id){
        return Message::where('from_id',Auth::user()->id)->where('to_id',$user_id)
                    ->orWhere('from_id',$user_id)->where('to_id',Auth::user()->id)->where('chat_type','private');
    }

    public function fetchMessagesGroupQuery($groupUser_id){
        return Message::where('to_id',$groupUser_id)->where('chat_type','group');
    }
    ...
    public function newMessage($data){
        ...
        $message->chat_type = $data['chat_type'];
        $message->save();
    }
    ...
    public function getLastMessageGroupQuery($groupUser_id){
        return self::fetchMessagesGroupQuery($groupUser_id)->orderBy('created_at','DESC')->latest()->first();
    }
    /**
    ...
    public function countUnseenMessages($user_id){
        return Message::where('from_id',$user_id)->where('to_id',Auth::user()->id)->where('chat_type','private')->where('seen',0)->count();
    }

    public function countUnseenGroupMessages($groupUser_id){
        return GroupMessageSeen::where('group_id',$groupUser_id)->where('user_id',Auth::user()->id)->where('seen',0)->count();
    }
    ...
    public function getGroupItem($messenger_id, $groupUser){
        // get last message
        $lastMessage = self::getLastMessageGroupQuery($groupUser->id);
        // Get Unseen messages counter
        $unseenCounter = self::countUnseenGroupMessages($groupUser->id);

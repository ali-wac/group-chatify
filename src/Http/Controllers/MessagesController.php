...
class MessagesController extends Controller
{
    ...
    {
        $path = storage_path() . '/storage/app/chat-attachments/' . $fileName;
        if (file_exists($path)) {
    ...
    public function send(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed)) {
                    ...
                    Storage::putFileAs('/chat-attachments', $file,$attachment);
                    // $file->storeAs("storage/app/chat-attachments", $attachment);
                } else {
        ...
        if (!$error_msg) {
            ...
            if($request['chatType'] == 'group'){
                $channelName = 'group-chatify';
                $message = Chatify::messageCardGroup($messageData, 'default');
            }else{
                ...
                Chatify::push($channelName, 'messaging', [
                    'from_id' => Auth::user()->id,
                    'to_id' => $request['id'],
                    'message' =>  $message
                ]);
    ...
    public function fetch(Request $request)
    {
        ...
        elseif($request['chatType'] == 'group'){
            ...
            if ($query->count() > 0) {
                foreach ($messages as $message) {
                    $allMessages .= Chatify::messageCardGroup(
                        Chatify::fetchMessage($message->id)
                    );
                }
                // send the response
    ...
    public function getGroups(Request $request)
    {
        ...
        $groupUsers = GroupUser::with('groups')->where('user_id',Auth::user()->id)->get();
        if ($groupUsers->count() > 0) {
            ...
            foreach ($groupUsers as $groupUser) {
                // if ($groupUser->user_id != Auth::user()->id) {
                    // Get user data
                    // $userCollection = User::where('id', $user->id)->first();
                    $groups .= Chatify::getGroupItem($request['messenger_id'], $groupUser);
                // }
            }
        }
        // send the response
        return Response::json([
            'groups' => $groupUsers->count() > 0 ? $groups : '<br><p class="message-hint"><span>Your contatct list is empty</span></p>',
        ], 200);
    }
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        for ($i = 0; $i < count($shared); $i++) {
            $sharedPhotos .= view('Chatify::layouts.listItem', [
                'get' => 'sharedPhoto',
                'image' => asset('storage/app/chat-attachments/' . $shared[$i]),
            ])->render();
        }
        // send the response
        return Response::json([
